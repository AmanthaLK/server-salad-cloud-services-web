...
- **Features section is duplicated verbatim from cpanel-hosting's
  ...
  same reuse-first approach as the hero above. Sits directly under the hero,
  above the tab switcher. `.cph-features`/`.cph-feature` are plain, unscoped
  classes with fully self-contained CSS (dark gradient band, responsive
  breakpoints included), so this needed no CSS changes at all — a markup-only
  copy-paste. Copy still describes cPanel Hosting specifically ("LiteSpeed
  Web Server", "WP Toolkit", "300+ web applications") and will need real
